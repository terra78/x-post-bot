...
- `priority_post_queue` にデータがあれば優先投稿（id降順）
...
`--account-slug`（または `ACCOUNT_SLUG`）は必須です。
...
外部cronサービスから `workflow_dispatch` を叩く想定で、`account_slug` を必ず渡します。
...
- `priority_post_queue`: 優先投稿キュー（`id` 降順、投稿成功で削除）
